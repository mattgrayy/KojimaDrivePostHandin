...
public class TypogenicText : MonoBehaviour
{
	...
	public void RebuildMesh()
	{
		...
		/*
		 * This may modify the original text's line break format, but to keep
		 * the glyph indices consistent, the split pattern has to be the
	...
	float BlitString(string str, float cursorX, float cursorY, List<int> vertexPointers = null)
	{
		...
		foreach (char c in str)
		{
			...
			if (inControlCode)
			{
				...
				{ 
					//no-op - fall through and produce backslash glyph
				} 
				else 
				{
					//Unrecognized code - warn and fall through to render
					Debug.LogWarning("Unrecognized control code: " + c);
					AddPlaceholderGlyphBounds();
					continue;
				}
			}
